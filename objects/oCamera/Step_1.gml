...
if !oConsole.console_status and room != rMenu_main and room != rMenu_settings and room != rMenu_keybinds {
	if mouse_wheel_down() {
		scroll_level -= .75
	}
	if mouse_wheel_up() {
...
if window_has_focus() {
	...
	//if room = rMenu_main or room = rMenu_settings or room = rMenu_keybinds { 
		//surface_resize(application_surface,window_get_width(),window_get_height())
	//}
}
...
//Move the zoom level based on mouse_ scrolling. Clamp the value so stuff doesn't get too silly.
